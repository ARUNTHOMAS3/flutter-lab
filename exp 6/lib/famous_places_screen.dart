...
class _FamousPlacesScreenState extends State<FamousPlacesScreen> {
  ...
  Future<void> _loadPlacesFromDB() async {
    final dbPlaces = await DatabaseHelper.instance.getAllPlaces();
    setState(() {
  ...
  Future<void> _addPlace() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_imageController.text.isEmpty) {
        _getImageFromWikipedia(placeName).then((imageUrl) async {
          if (imageUrl != null && mounted) {
            setState(() {
              places.last =
                  Place(name: newPlace.name, country: newPlace.country, image: imageUrl, latLng: newPlace.latLng);
            });
            await DatabaseHelper.instance.updatePlace(places.last);
          }
        });
      }

      await DatabaseHelper.instance.insertPlace(newPlace);
